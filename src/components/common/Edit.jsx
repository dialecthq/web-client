...
const Edit = ({ children, setEditing, initialValues, index }) => {
  ...
  const onFinish = (values) => {
    ...
    if (Object.keys(parameters).includes("language")) {
      ...
      if (
        user.languages.some(
          (e) => e.key === rooms.filter((e) => e.value === parameters.language)[0].key
        )
      ) {
        console.log(user.languages)
        message.error({
          content: "Cannot have the same language twice",
      ...
      tempLanguages[index] = {
        ...
        key: rooms.filter((e) => e.value === parameters.language)[0].key
      }
