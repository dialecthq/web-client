...
const Edit = ({ children, setEditing, initialValues, index }) => {
  ...
  const onFinish = (values) => {
    ...
    if (Object.keys(parameters).includes("country")) {
      parameters = {
        country: countryOptions.filter((e) => e.value === values.country)[0]
          .key,
      };
    }
    ...
    if (Object.keys(parameters).includes("language")) {
      ...
      tempLanguages[index] = {
        level: parseInt(
          levelOptions.filter((e) => e.value === parameters.level)[0].key,
          10
        ),
        key: languageOptions.filter((e) => e.value === parameters.language)[0]
          .key,
      };
